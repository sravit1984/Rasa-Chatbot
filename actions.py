...
class ActionSearchRestaurants(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		average_cost = tracker.get_slot('cuisine')
		...
		if (loc in tier_1) | (loc in tier_2):
			...
			cuisines_dict={'chinese':25, 'mexican':73, 'italian':55, 'american':136,'south indian':85,'north indian':50}
			...
			df['Ratings'] = df['Ratings'].convert_objects(convert_numeric=True)
			df = df.sort_values(by='Ratings', ascending=0)

			response = display_response(df)
			dispatcher.utter_message("-----"+response)
			email_msg = email_response(df)
			return [SlotSet('email_message',email_msg)]
			#for index, row in df.iterrows():
			#	response = response + row['Name'] + " " + row['Address'] + " " + str(row['Ratings']) + " " + str(row['Average_cost']) + "\n"
...
class ActionSendEmail(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		message = tracker.get_slot('email_message')
